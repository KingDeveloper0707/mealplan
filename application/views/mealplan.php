...
$has_mar = false;
$has_ido_video = false;
$has_ebook_easy_keto = false;
$has_ebook_ez_grocery = false;
$has_ebook_fat_burning = false;
$has_ebook_how_to_stay = false;
$has_ebook_Interrmittent_fasting = false;
$has_ebook_keto_comfort = false;
$has_ebook_holiday_cookbook = false;
$has_ebook_workout_at_home = false;
$has_ebook_italian_keto = false;

foreach ($purchased_products as $obj_product) {
    if ($obj_product->is_subscription) {
        $has_subscription = true;
    }
    if ($obj_product->weeknum == 12 && !$obj_product->is_subscription) {
        $has_12week = true;
    }
    if ($obj_product->weeknum == 52 && !$obj_product->is_subscription) {
        $has_12month = true;
    }
    if ($obj_product->is_orderbump && $obj_product->weeknum > 0) {
        $has_orderbump = true;
    }
    if ($obj_product->is_upsell && $obj_product->weeknum > 0) {
        $has_upsell_mp = true;
    }
    if ($obj_product->title == "Metabolic Age Reverser") {
        $has_mar = true;
    }
    if ($obj_product->title == "Keto Video Coaching with Ido") {
        $has_ido_video = true;
    }
    if ($obj_product->title == "Easy Keto Treats eBook") {
        $has_ebook_easy_keto = true;
    }
    if ($obj_product->title == "EZ Grocery List eBook") {
        $has_ebook_ez_grocery = true;
    }
    if ($obj_product->title == "Fat Burning Desserts eBook") {
        $has_ebook_fat_burning = true;
    }
    if ($obj_product->title == "How To Stay On Your Plan When Dining Out eBook") {
        $has_ebook_how_to_stay = true;
    }
    if ($obj_product->title == "Interrmittent Fasting Guide eBook") {
        $has_ebook_Interrmittent_fasting = true;
    }
    if ($obj_product->title == "Keto Comfort Foods eBook") {
        $has_ebook_keto_comfort = true;
    }
    if ($obj_product->title == "The Holiday Cookbook eBook") {
        $has_ebook_holiday_cookbook = true;
    }
    if ($obj_product->title == "Workout At Home eBook") {
        $has_ebook_workout_at_home = true;
    }
    if ($obj_product->title == "Italian Keto Cookbook") {
        $has_ebook_italian_keto = true;
    }
}
?>
        ...
        <div class="collapse navbar-collapse" id="navbarResponsive">
            ...
            <ul class="navbar-nav ml-auto">
                <li class="nav-item <?php if ($page == "mealplan") echo "active"; ?>">
                    <a class="nav-link js-scroll-trigger" href="<?php echo base_url() ?>mealplan?uid=<?php echo $uid; ?>&page=mealplan" style="width:115px;">MEAL PLAN</a>
                </li>
                <?php
                if ($has_mar) {
                    ?>
                    <li class="nav-item hide-fastspring <?php if ($page == "mar") echo "active"; ?>">
                        <a class="nav-link js-scroll-trigger" href="<?php echo base_url() ?>mealplan?uid=<?php echo $uid; ?>&page=mar&subpage=course" style="width:170px;">METABOLIC AGE REVERSER</a>
                    </li>
                    <?php
                }
                if ($has_orderbump || $has_ido_video) {
                    ?>
                    <li class="nav-item hide-fastspring <?php if ($page == "video") echo "active"; ?>">
                        <a class="nav-link js-scroll-trigger" href="<?php echo base_url() ?>mealplan?uid=<?php echo $uid; ?>&page=video" style="width:157px;">VIDEO COACHING</a>
                    </li>
                <?php } ?>
                <li class="nav-item <?php if ($page == "download") echo "active"; ?>">
                    <a class="nav-link js-scroll-trigger" href="<?php echo base_url() ?>mealplan?uid=<?php echo $uid; ?>&page=download" style="width:118px;">DOWNLOAD</a>
                </li>
                <li class="nav-item <?php if ($page == "resource") echo "active"; ?>">
                    <a class="nav-link js-scroll-trigger" href="<?php echo base_url() ?>mealplan?uid=<?php echo $uid; ?>&page=resource" style="width:120px;">RESOURCES</a>
                </li>
                <li class="nav-item <?php if ($page == "faq") echo "active"; ?>">
                    <a class="nav-link js-scroll-trigger" href="<?php echo base_url() ?>mealplan?uid=<?php echo $uid; ?>&page=faq" style="width:63px;">FAQ</a>
                </li>
                <li class="nav-item hide-fastspring <?php if ($page == "vip") echo "active"; ?>">
                    <a class="nav-link js-scroll-trigger" href="<?php echo base_url() ?>mealplan?uid=<?php echo $uid; ?>&page=vip" style="width:130px;">VIP SPECIALS</a>
                </li>
            </ul>
        </div>
    </div>
...
<header class="masthead <?php echo $page; ?>">
    <?php if ($page == "mealplan") { ?>
        <h1>
            <span class="mealplan_firstname">
                <?php echo $first_name && strlen($first_name) > 0 ? trim($first_name) . "'s" : "My" ?></span>
            ...
            <div class="plan-area">
                <form class="o-meal-table-head__meal-weeks m-meal-weeks js-week-form">                        
                    <div class="weeks-container">
                        <select id="demo-select" onchange="weekChange(this.value)">
                            <?php
                            $weeknum = count($obj_mealplan) / 7 / 4;
                            for ($i = 1; $i <= $weeknum; $i++) {
                                echo '<option value="' . ($i - 1) . '">Select Week ' . $i . '</option>';
                            }
                            ?>
                        </select>
                    </div>
    ...
    <?php } ?>
    <?php if ($page == "vip" && !$has_12month && !$has_subscription) { ?>
...
<?php if ($page == "mealplan") { ?>
    <section id="download">
        <div class="d-none d-md-block">
            ...
            <a href="api/download_grocery_list" class="btn btn-primary btn-download mt-3 mt-sm-0">
            ...
            <div class="row no-gutters">
                <div class="col-12 pl-0 pr-0 pl-md-auto pr-md-auto">                        
                    ...
                    <div class="quickstart-content">
                        ...
                        <p>That's why we made <span class="proxima-nova-bold">Keto Shake</span>.  It's a creamy, sweet milkshake that will NEVER hurt your KETO. It's made with whey protein and Avocado Oil. Organic Agave - powerful prebiotics, and ultra purified MCT Oil. It's Konscious Brand, so you the ingredients are second to none, and be prepared to save money by choosing a multi-pack.</p>
                        <p class="proxima-nova-bold">100% Money Back Guarantee.  Try it for 30 days and use coupon code <span class="red-color" style="font-style: initial;">Welcome10</span> to save 10% on your order. 100% Natural, comes in 3 different flavors, with no artificial anything. <a href="https://konsciousketo.com/products/konscious-keto-shake" class="blue-color" target="_blank">Click here</a> to claim yours now.</p>
            ...
            <div class="row text-center no-gutters">
                <div class="col-12 pl-0 pr-0 pl-md-auto pr-md-auto">                        
                    ...
                    <div class="faq-content">
                        ...
                        <div class="panel">
                            <div class="pt-3 pb-3">
                                <p>If you purchased <a href="https://konsciousketo.com/products/konscious-keto-shake" target="_blank">Keto Shake</a> or <a href="https://konsciousketo.com/pages/keto-activate-gc" target="_blank">Keto Activate</a> with your custom meal plan, we are preparing your order right away and will send you a confirmation email (with a tracking ID) when those items leave our warehouse. All items ship the following business day.</p>
                        ...
                        <div class="panel">
                            <div class="pt-3 pb-3">
                                <p>If you purchased any <a href="https://konsciousketo.com/products/konscious-keto-shake" target="_blank">Keto Shakes</a> to complement your plan, you can use them to replace your breakfast, lunch or dinner as needed. Simply omit the recipe for that time and make a delicious, creamy shake in mere minutes.</p>
                                ...
                                <p><a href="https://konsciousketo.com/products/konscious-keto-shake" target="_blank">Keto Shake</a> is a delicious, dessert-like meal replacement that will help you eat more healthy fats and curb cravings and can be enjoyed as a light meal or keto snack at anytime of the day.</p>
                        ...
                        <div class="panel">
                            <div class="pt-3 pb-3">
                                ...
                                <p>8. Keto-friendly protein shakes such as <a href="https://konsciousketo.com/products/konscious-keto-shake" target="_blank">keto shake</a> are an excellent choice as you'll get hydration and the fat along with protein will reduce your hunger.</p>
                                <p></p>
                            </div>
                        </div>
                        ...
                        <?php
                        if (!$has_subscription && !$has_12month && !$has_12week) {
                            ?>
                            <button class="accordion">What Happens After 28 Days?<svg class="icon"><use xlink:href="#angle-down"></use></svg></button>                        
                            <div class="panel hide-subscription">
                                <div class="pt-3 pb-3">
        ...
        <div class="row justify-content-center intermediate hide-fastspring">
            <div class="col-6 col-lg-3 py-3">
                <div class="item locked mp-ebook-5">
                    ...
                    <?php if ($has_upsell_mp || $has_ebook_Interrmittent_fasting) { ?>
            ...
            <div class="col-6 col-lg-3 py-3">
                <div class="item locked mp-ebook-6">
                    ...
                    <?php if ($has_upsell_mp || $has_ebook_how_to_stay) { ?>
            ...
            <div class="col-6 col-lg-3 py-3">
                <div class="item locked mp-ebook-7">
                    ...
                    <?php if ($has_upsell_mp || $has_ebook_fat_burning) { ?>
        ...
        <div class="row advanced hide-fastspring">
            <div class="col-6 col-lg-3 py-3">
                <div class="item locked mp-ebook-9">
                    ...
                    <?php if ($has_orderbump || $has_ebook_workout_at_home) { ?>
            ...
            <div class="col-6 col-lg-3 py-3">
                <div class="item locked mp-ebook-11">
                    ...
                    <?php if ($has_orderbump || $has_ebook_easy_keto) { ?>
            ...
            <div class="col-6 col-lg-3 py-3">
                <div class="item locked mp-ebook-10">
                    ...
                    <?php if ($has_orderbump || $has_ebook_italian_keto) { ?>
            ...
            <div class="col-6 col-lg-3 py-3">
                <div class="item locked mp-ebook-12">
                    ...
                    <?php if ($has_orderbump || $has_ebook_keto_comfort) { ?>
    ...
    <section id="vip_product">
        <div class="container">
            <div class="row">
                ...
                <div class="col-12 col-lg-4">
                    <a href="https://konsciousketo.com/pages/metabolic-energizer" class="item d-inline-block" target="_blank">
                        <p class="text text-center blue-color proxima-nova-bold mb-3">Try Metabolic Energizer</p>
                        <picture>
                            <source srcset="<?php echo keycdn_url() . 'assets/img/'; ?>mp_me_1.webp" type="image/webp">
                            <img class="mx-auto d-block w-100" src="<?php echo keycdn_url() . 'assets/img/'; ?>mp_me_1.jpg" alt="">
                        </picture>
                    </a>
                </div>
                <div class="col-12 col-lg-4">
...
<?php if ($page == "mar") { ?>
    <div id="wrapper">
        <ul class="navbar-nav bg-white sidebar sidebar-light py-5" id="accordionSidebar">

            <!-- Nav Item - Dashboard -->
            <li class="nav-item <?php if ($subpage == "course") echo "active"; ?>">
                <a class="nav-link" href="<?= base_url() ?>mealplan?uid=<?= $uid ?>&page=mar&subpage=course">
                    <span>Course</span></a>
            </li>
            <li class="nav-item <?php if ($subpage == "guides") echo "active"; ?>">
                <a class="nav-link" href="<?= base_url() ?>mealplan?uid=<?= $uid ?>&page=mar&subpage=guides">
                    <span>Guides</span></a>
            </li>

            <li class="nav-item <?php if ($subpage == "fat_burner") echo "active"; ?>" style="display: none;">
                <a class="nav-link" href="<?= base_url() ?>mealplan?uid=<?= $uid ?>&page=mar&subpage=fat_burner">
                    <span>21-Day Fat Burner</span></a>
            </li>

            <li class="nav-item <?php if ($subpage == "qa") echo "active"; ?>">
                <a class="nav-link" href="<?= base_url() ?>mealplan?uid=<?= $uid ?>&page=mar&subpage=qa">
                    <span>Questions & Answers</span></a>
            </li>

            <li class="nav-item d-none <?php if ($subpage == "rv") echo "active"; ?>">
                <a class="nav-link" href="<?= base_url() ?>mealplan?uid=<?= $uid ?>&page=mar&subpage=rv">
                    <span>Recipe Videos</span></a>
            </li>
        ...
        <div id="content-wrapper">
            <div id="content">
                <?php if ($subpage == "course" || $subpage == "fat_burner") { ?>
                    <div class="container-fluid video-container <?php echo $subpage; ?>">
                        <div class="row">
                            <div class="col-12 col-lg-9">
                                <div class="d-flex flex-row pt-5 pb-3">
                                    <div class='w-100'>
                                        <div class="wistia_responsive_padding" style="position:relative;padding:56.25% 0 0 0;position:relative;flex:1 1 auto;">
                                            <div class="wistia_responsive_wrapper" style="height:100%;left:0;position:absolute;top:0;width:100%;">
                                                <div class="wistia_embed wistia_async_<?php echo $subpage == "course" ? "3oopr5lc1p" : "zxo7e4eazc"; ?> videoFoam=true autoPlay=false" style="height:100%;position:relative;width:100%">&nbsp;</div>                            
                                            </div>                                
                                        </div>
                                        <div class="video-heading mt-3 mb-3 d-flex">
                                            <div class="logo-img">
                                                <img src="<?php echo keycdn_url() . 'assets/img/'; ?>logo-mp.svg" width="190" class="w-100" />
                                            </div>
                                            <div class="d-inline-block collapse-wrapper">
                                                <h3 class="vid-title blue-color proxima-nova-bold mt-2"><?php echo $subpage == "course" ? "Finding the Fountain of Youth Through Your Diet" : "Day 1"; ?></h3>
                                                <?php if ($subpage == "course") { ?>
                                                    <p class='vid-content-visible'>Very few people are aware that the fountain of youth is real, and that one of the tickets to the fountain of youth is through your diet.</p>
                                                    <div class="vid-content collapse vid-content-hidden" id="collapseContent" aria-expanded="false">
                                                        <p>Our western approach to diet is complex and, in many ways, counter-productive. The way that we eat is just as important as what we eat, and you might be surprised to learn that you can seriously improve your health just by changing the time of day that you choose to eat your meals.</p>
                                                    ...
                                                    <a role="button" class="collapsed btn-showmore" data-toggle="collapse" href="#collapseContent" aria-expanded="false" aria-controls="collapseContent">Show <span class='span-more'>More</span><span class='span-less'>Less</span></a>
                                                <?php } ?>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="ads-greens p-3 d-none">
                                        <h3 class="red-color h4">Detoxify Your Body With This All-In-One Superfood Drink</h3>
                                        <p class="black-color">Get Your Daily Greens WITHOUT Shopping, Chopping... Or The Messy Clean-Up!</p>

                                        <picture>
                                            <source srcset="<?php echo keycdn_url() . 'assets/img/'; ?>mar-green-product-min.webp" type="image/webp">
                                            <img class="mx-auto d-block w-100" src="<?php echo keycdn_url() . 'assets/img/'; ?>mar-green-product-min.jpg" alt="">
                                        </picture>
                                        <div class="d-flex">
                                            <a href="https://konsciousketo.com/pages/green" class="btn btn-primary btn-red mt-3 mx-auto" target="_blank">CLAIM YOURS TODAY</a> 
                                        </div>
                                    </div>
                                </div>

                                <div class="vid-carousel"></div>
                            </div>
                        </div>
                    </div>
                <?php } ?>
                <?php if ($subpage == "guides") { ?>
                    <div class="container my-5">
                        <div class="row">
                            <div class="col-12">
                                <div class="d-flex flex-column flex-md-row justify-content-between align-items-center">
                                    <div class="text-wrapper px-3">
                                        <h3 class='h4 blue-color'>MAR User Guide</h3>
                                        <p class='text-dark'>This user-friendly book will teach you everything you need to know about Metabolic Age Reversal: how to look younger, feel younger, and act younger. This book shares all the science while separating fact from fiction, so you know what’s real.</p>
                                        <div class='indicator-line'></div>
                                        <a href="MAR/mar-user-guide.pdf" class="btn btn-primary btn-red mt-3 mx-auto downloadable btn-download" target="_blank">Download >></a>                                        
                                    </div>
                                    <picture>
                                        <source srcset="<?php echo keycdn_url() . 'assets/img/'; ?>mar-user-guide-1.webp" type="image/webp">
                                        <img class="mx-auto d-block px-3" src="<?php echo keycdn_url() . 'assets/img/'; ?>mar-user-guide-1.jpg" alt="" width="240">
                                    </picture>
                                </div>
                                <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mt-3">
                                    <div class="text-wrapper px-3">
                                        <h3 class='h4 blue-color'>MAR Recipe Guides</h3>
                                        <p class='text-dark'>This e-book provides you with all the information you will need to prepare your own Metabolic Age Reversal recipes. Our keto-friendly soups and drinks will keep you healthy and energized throughout your fast.</p>
                                        <div class='indicator-line'></div>
                                        <a href="MAR/mar-recipes-guide.pdf" class="btn btn-primary btn-red mt-3 mx-auto downloadable btn-download" target="_blank">Download >></a>
                                    </div>
                                    <picture>
                                        <source srcset="<?php echo keycdn_url() . 'assets/img/'; ?>mar-recipes-guide-1.webp" type="image/webp">
                                        <img class="mx-auto d-block px-3" src="<?php echo keycdn_url() . 'assets/img/'; ?>mar-recipes-guide-1.jpg" alt="" width="240">
                                    </picture>
                                </div>
                                <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mt-3">
                                    <div class="text-wrapper px-3">
                                        <h3 class='h4 blue-color'>MAR Quick Start Guide</h3>
                                        <p class='text-dark'>All the information that you’ll need to reference for your fast - compacted into a simple one-page graphic.</p>
                                        <div class='indicator-line'></div>
                                        <a href="MAR/mar-quick-start-guide.pdf" class="btn btn-primary btn-red mt-3 mx-auto downloadable btn-download" target="_blank">Download >></a>
                                    </div>
                                    <picture>
                                        <source srcset="<?php echo keycdn_url() . 'assets/img/'; ?>mar-quick-start-guide-1.webp" type="image/webp">
                                        <img class="mx-auto d-block px-3" src="<?php echo keycdn_url() . 'assets/img/'; ?>mar-quick-start-guide-1.jpg" alt="" width="240">
                                    </picture>
                                </div>
                            </div>
                        </div>
                ...
                <?php if ($subpage == "qa") { ?>
                    <div class="container my-5">
                        <div class="row">
                            <div class="col-12">
                                <div class="faq-content">
                                    <button class="accordion">Do you eat breakfast, lunch, dinner, and snack in the eight hours of eating? Or do you skip a meal?</button>
                                    <div class="panel">                                        
                                        <p>For example, if you fast between bedtime and 12 noon, can you eat all three meals and snack by 8pm?</p>
                                        <p>We find most of our customers choose to skip a meal and have their pick of breakfast or lunch, a snack, and dinner. If you do this, it can be a good idea to add an extra half serving to the first meal of your day.</p>
                                        <p>If you prefer to have 3 main meals and a snack, that is A-OK too! We like to give you a lot of flexibility to tweak and adjust the plan to suit your needs.</p>
                                        <p>If you aren’t sure what to do, then we recommend you pick to eat either breakfast or lunch from your meal plan - whichever looks more delicious to you - as well as a snack and dinner. You may want to have an extra half serving of each main meal to ensure you are eating enough.</p>                                        
                                    </div>

                                    <button class="accordion">How long do I fast? Days? Weeks?</button>
                                    <div class="panel">                                        
                                        <p>We recommend gradually increasing your fasting time over 2-3 months until you reach a daily 16 hour-fasting period. This gives your body time to adjust to being in ketosis. Fasting on the average, typically high carb diet is challenging - fasting when in ketosis is pretty effortless.</p>
                                        <p>So don’t rush into fasting until your body is keto-adapted, which for most people happens in 2-6 weeks, depending on what your diet was like prior to keto. The more carbs you ate before, the longer it usually takes to become keto-adapted. Being keto-adapted means, you find keto easy and don’t feel any carb cravings.</p>
                                        <p>We find that 16:8 fasting is the best for weight loss, anti-aging, boosting your metabolism, and boosting your immune system. It’s also the most sustainable and easiest way to stick to fasting. We don’t recommend longer fasts except a possible quarterly 3-day fast if you feel up for it. We don’t recommend going longer than that, though.</p>
                                        <p>We find other methods of intermittent fasting such as 5:2 where you eat normally for 5 days and don’t eat for 2 days, are too challenging and often difficult to enjoy.</p>
                                    </div>

                                    <button class="accordion">Can I eat or drink during the fasting period?</button>
                                    <div class="panel">                                        
                                        <p>There are 3 methods of fasting and you are free to choose which you prefer. We’ll give a quick summary of the pros and cons for each:</p>
                                        <p><strong>Water fasting</strong> - a fast where you only drink water and nothing else during your fasting period</p>
                                        <div class='ml-3'>
                                            <p>Pros:</p>
                                            <ul>
                                                <li>Water fasting is the most extreme type of fasting.</li>
                                                <li>It delivers the max amount of benefits.</li>
                                                <li>Water fasting works very quickly.</li>
                                            </ul>
                                            <p>Cons:</p>
                                            <ul>
                                                <li>Water fasting can be uncomfortable.</li>
                                                <li>It is often difficult to sustain.</li>
                                                <li>Requires the most self-discipline (though many people would consider this a benefit since fasting is also a good way to train willpower through discipline).</li>
                                            </ul>
                                        </div>

                                        <p><strong>Under 50 calorie fasting</strong> - a fast where you consume no more than 50 calories during your fasting period and can drink liquids such as coffee, tea, straight bone broth, calorie-free electrolytes and Keto Activate.</p>
                                        <div class='ml-3'>                                            
                                            <p>Pros:</p>
                                            <ul>
                                                <li>Much more enjoyable and comfortable than water fasting.</li>
                                                <li>Lowering your calories leads to a deep ketosis in your fasted state.</li>
                                                <li>You enjoy pretty much all the benefits of fasting.</li>
                                                <li>A fairly sustainable approach to follow.</li>
                                            </ul>
                                            <p>Cons:</p>
                                            <ul>
                                                <li>Still requires a fair bit of self-discipline.</li>
                                                <li>When combined with the keto diet, the under 50 calorie fast can be tough on your body and make some people irritable.</li>
                                                <li>Can still prove a little challenging for some to follow.</li>
                                            </ul>
                                        </div>
                                        <p><strong>Supported fasting</strong> - this is the specific program we describe in this Metabolic Age Reverser product where you can have near unlimited amounts of bone broth, bone broth based soups, coffee and tea during your fasting period, along with products such as electrolytes and coconut/MCT oil.</p>
                                        <div class='ml-3'>
                                            <p>Pros:</p>
                                            <ul>
                                                <li>The easiest way to fast.</li>
                                                <li>The most enjoyable, comfortable and sustainable way to fast.</li>
                                                <li>You’re getting the benefits of fasting while limiting hunger and cravings.</li>
                                                <li>You can keep energy levels high.</li>
                                                <li>Not putting much strain on your body.</li>
                                            </ul>

                                            <p>Cons:</p>
                                            <ul>                                            
                                                <li>Fasting purists might complain it’s not hardcore enough for them ;)</li>
                                            </ul>
                                        </div>
                                    </div>

                                    <button class="accordion">Isn’t it unhealthy to skip breakfast?</button>
                                    <div class="panel">                                        
                                        <p>Although we’ve often heard the myth that breakfast is the most important meal of the day, we never actually ate a set, regular breakfast for most of human history. Many tribes still living an authentic ancestral way of life don’t even have a word for breakfast.</p>
                                        <p>The reason why research into breakfast is misleading is that many unhealthy people often skip breakfast. People frequently make the faulty conclusion that skipping breakfast is unhealthy when the many breakfast skippers are either drinking heavily during the week or having other harmful habits, causing them to rush in the morning and skip breakfast.</p>
                                        <p>The research is beginning to pile up that skipping breakfast as part of an intermittent fasting regime is incredibly good for you.</p>
                                    </div>

                                    <button class="accordion">Can I work out while fasting?</button>
                                    <div class="panel">
                                        <p>Working out while in a fasted state can lead to incredible fat burning levels and losing weight because you tap straight into your body’s fat stores. However, we recommend a gentle activity such as walking, yoga, stretching, and light movement.</p>
                                        <p>Higher intensity exercise isn’t out of the question if you really want to. However, just be cautious of not putting too much strain on your body. This will drive up your hunger and lower your energy levels, both of which can present problems if you’re trying to stay fasted. We find that a lower intensity movement that feels good is the best choice.</p>
                                        <p>If you want to do some kind of resistance or weight training, this is a fantastic way to lose weight healthily. This can take years off your biological clock and boosts your metabolism. We highly recommend this if it appeals to you.</p>
                                    </div>

                                    <button class="accordion">Will fasting cause muscle loss?</button>
                                    <div class="panel">
                                        <p>While it’s true that pretty much all weight loss programs have a risk of causing muscle loss, one study shows that you’re less likely to lose muscle mass during intermittent fasting <a href="https://www.ncbi.nlm.nih.gov/pubmed/21410865" target="_blank">compared to</a> regular calorie restriction.</p>
                                        <p>If you are concerned about muscle loss during fasting, you’ll find bone broth, and bone broth-based soups will give you a boost of protein and collagen, which will help prevent any muscle loss. If you prefer, you can also include a branch chained amino acid supplement during your fasting period to avoid muscle loss as well.</p>
                                        <p>In our experience, muscle loss is more of an issue during fasts that last for more than 24 hours. Since your fasting window during this program will only be for 16 hours, you probably do not have to worry.</p>
                                    </div>

                                    <button class="accordion">Will fasting slow down my metabolism?</button>
                                    <div class="panel">
                                        <p>No. Studies show that short-term fasts actually boost metabolism.</p>
                                        <p>You might think that since you’re eating less food, your body will respond by slowing down your metabolism. Lucky for you, this isn’t the case. Intermittent fasting is known to increase your metabolism, and <a href="https://www.ncbi.nlm.nih.gov/pubmed/2405717" target="_blank">even 48-hour fasts</a> can increase your metabolism.</p>
                                        <p>Many <a href="https://pubmed.ncbi.nlm.nih.gov/10837292/" target="_blank">studies</a> prove that short-term fasting actually increases the amount of calories that you burn at rest. In other words, it increases your metabolism. You’ll be burning fat while you’re not even doing anything.</p>
                                        <p>One of the ways that this works is by improving certain hormones in your body. Your metabolism relies on hormones to do different things, including burn fat. Intermittent fasting improves the way that these hormones work.</p>
                                        <p>One of the most important hormones for this is insulin. Most people know that insulin is related to diabetes, but it’s also important for weight loss. If you have too much insulin in your body, it tells your body to store fat and prevents it from breaking down fat.</p>
                                        <p>Intermittent fasting <a href="https://www.ncbi.nlm.nih.gov/pubmed/15640462" target="_blank">can be useful</a> for helping to reduce the amount of insulin in your body, which reduces fat and therefore increases your metabolism.</p>
                                        <p>It’s important to be aware of how long that you’re fasting for, though. Long fasts - lasting 3 days or longer - can <a href="https://www.healthline.com/nutrition/starvation-mode" target="_blank">suppress</a> metabolism, which is the opposite of what you want.</p>                                        
                                    </div>

                                    <button class="accordion">Should kids fast?</button>
                                    <div class="panel">                                        
                                        <p>No, we don’t recommend fasting for children.</p>
                                    </div>

                                    <button class="accordion">When do I start timing my 16 hour-fasting window?</button>
                                    <div class="panel">
                                        <p>Do I start counting my 16 hours when I start my last meal or after it’s finished? If I start dinner at 8pm and finish by 9pm, when does the 16 hours start?</p>
                                        <p>Your fasting period starts from when you finish eating. So in this example, it would be 9pm.</p>                                        
                                    </div>

                                    <button class="accordion">What are the different types of intermittent fasting?</button>
                                    <div class="panel">                                        
                                        <p>The basics of intermittent fasting are actually the same: you limit the amount of time each day that you eat. However, some people find it easier to do this in different ways. Here are the most popular different intermittent fasting schedules.</p>
                                        <ul>
                                            <li>
                                                <p><strong>The 16:8 method.</strong> This is by far the most popular style of intermittent fasting. The 16:8 method is simple: you will fast for <a href="https://www.everydayhealth.com/diet-nutrition/diet/fasting-diet-may-help-with-weight-loss-lower-blood-pressure/" target="_blank">16 hours</a> each day, allowing yourself an 8 hour window in which you can eat food.</p>
                                                <p>Most people find that the easiest way to do this is to skip breakfast, or simply bump it back to a late brunch. By doing this - basically having a late brunch - and having dinner at a reasonable hour (between 6 and 7) it’s actually pretty easy to follow the 16:8 schedule.</p>
                                                <p>For example, if you have your window set between 11 AM and 7 PM, you can enjoy brunch at 11 AM, have a mid-day meal, and enjoy dinner at 7 PM. Other people prefer to have their eating window between noon and 8. According to intermittent fasting advocate Dr. Lowden, this approach is also often called “time-restricted eating.”</p>
                                            </li>
                                            <li>
                                                <p><strong>Alternate-day fasting.</strong> This method is a bit different, and involves eating significantly less for a whole day followed by eating a normal amount the following day. According to a <a href="https://www.sciencedirect.com/science/article/pii/S193152441400200X" target="_blank">study published in October 2014 in Translational Research</a>, an effective way to do this is by limiting your calories to 25 percent on your fasting days.</p>
                                                <p>Some people prefer to break up their week by fasting on Mondays, Wednesdays, and Fridays, and by eating regularly on weekends and the other weekdays. We generally don’t recommend this as it can be tough to stick to and if you’re new to fasting it can be rather uncomfortable.</p>
                                            </li>
                                            <li>
                                                <p><strong>5:2 fasting.</strong> This is another form of intermittent fasting in which people choose to restrict calories for two nonconsecutive days each week. A very low number of calories (around 400 to 500 calories) is allowed on these two fasting days, and the other five days have no eating restrictions, according to the definition provided by the <a href="https://www.hsph.harvard.edu/nutritionsource/healthy-weight/diet-reviews/intermittent-fasting/" target="_blank">Harvard T.H. Chan School of Public Health</a>.  Some people find this type of fasting enjoyable, but again, we don’t recommend it for beginners.</p>
                                            </li>
                                        </ul>
                                    </div>

                                    <button class="accordion">Can intermittent fasting help you lose weight?</button>
                                    <div class="panel">
                                        <p>Intermittent fasting can certainly help you lose weight. There are lots of studies that prove this, and thousands of people around the world have all agreed that intermittent fasting is great for weight loss.</p>
                                        <p>Here are a couple of the most prominent studies.</p>
                                        <ul>
                                            <li>
                                                <p>Intermittent fasting can help to boost norepinephrine, which is an important hormone. Also known as adrenaline, this hormone helps to boost your metabolism. This means that with more norepinephrine you’ll be burning more calories even at rest, and IF can <a href="https://www.ncbi.nlm.nih.gov/pubmed/10837292" target="_blank">help with that</a>.</p>
                                            </li>
                                            <li>
                                                <p>Intermittent fasting also helps to improve insulin levels in your body. Insulin is a hormone that tells your body to store fat, so if you have lots of insulin, you’ll probably also have lots of fat. Intermittent fasting can <a href="https://www.ncbi.nlm.nih.gov/pubmed/15640462" target="_blank">reduce</a> the amount of insulin present in your body, thereby making it easier for you to lose weight.</p>
                                            </li>
                                            <li>
                                                <p>Intermittent fasting can also be useful for <a href="https://www.ncbi.nlm.nih.gov/pubmed/21410865" target="_blank">helping</a> you hold onto lean muscle mass. The more muscle you have, the more calories you’ll burn and therefore the more weight you’ll lose.</p>
                                            </li>
                                            <li>
                                                <p>One review pointed out that people who are following an intermittent fasting routine may lose as much as 8% of their total body weight and as much as 16% of total body fat over <a href="https://www.ncbi.nlm.nih.gov/pubmed/21410865" target="_blank">3-12 weeks</a>.</p>
                                            </li>
                                            <li>
                                                <p>Intermittent fasting has also been shown to combine well with the keto diet, which is another fantastic tool for losing weight. People who practice IF are less likely to have to deal with the unpleasantness of <a href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4313585/" target="_blank">keto flu</a>.</p>
                                            </li>
                                        </ul>
                                    </div>

                                    <button class="accordion">What are the touted benefits of intermittent fasting, and are they legit?</button>
                                    <div class="panel">
                                        <p>Here are some of the proposed benefits of IF:</p>
                                        <ul>
                                            <li>
                                                <p><strong>IF boosts weight loss.</strong> Another <a href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5959807/" target="_blank">review published in Current Obesity Reports in June 2018</a> agrees that most of the studies done on IF report that it can help with weight loss. According to this review, people who practice IF for a period of weeks can lose somewhere between 5-9.9% of their body weight.</p>
                                                <p>Another study, this one <a href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6004924/" target="_blank">published</a> in 2018 in the journal Nutrition and Healthy Aging, suggests alternate-day fasting may lead to even more weight loss than time-restricted eating. However, alternate-day fasting may be harder to stick with than time-restricted eating.</p>
                                            </li>
                                            <li>
                                                <p><strong>Increase longevity.</strong> A <a href="https://www.cell.com/cell/fulltext/S0092-8674(14)01525-6" target="_blank">study published in Cell in December 2014</a> reports that calorie restriction may slow aging. The study involved animals, however, and the same findings haven’t been shown among humans yet. The general idea, however, is that the body activates a cellular repair process when we go long periods without eating. This keeps us in good shape.</p>
                                            </li>
                                            <li>
                                                <p><strong>Reduced insulin resistance.</strong> Insulin resistance is the hallmark of <a href="https://www.everydayhealth.com/type-2-diabetes/guide/" target="_blank">type 2 diabetes</a> and tends to happen when we produce too much insulin or when our body doesn’t respond to the insulin that we do produce. Being overweight increases the likelihood of insulin resistance, <a href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3936017/" target="_blank">according to a study published in February 2014 in Frontiers of Medicine</a>.</p>
                                                <p>IF can help with insulin resistance by reducing the number of calories consumed overall, <a href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6521152/" target="_blank">according to a study published in Nutrients in April 2019</a>. IF also directly reduces the amount of insulin produced, according to the study linked to in the previous section. It’s important to note that diabetics may want to be cautious with fasting, since reduced insulin is not always in their best interest.</p>
                                            </li>
                                            <li>
                                                <p><strong>Improved heart health.</strong> A <a href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3511220/" target="_blank">study published in Nutrition Journal</a> reported that IF helped the participants lose weight and improved their overall cholesterol levels. Obesity and high cholesterol are both risk factors for coronary disease, which led researchers to conclude that IF can help reduce the risk of these <a href="https://www.everydayhealth.com/coronary-artery-disease/guide/" target="_blank">illnesses</a>.</p>
                                            </li>
                                            <li>
                                                <p><strong>Improved metabolism</strong>.  Metabolic parameters - measurements regarding your metabolism - tend to reflect your weight. According to Dr. Lowden, “a lot of those <a href="https://www.everydayhealth.com/diet-nutrition/intermittent-fasting-may-improve-metabolic-health-small-study-finds/" target="_blank">metabolic parameters respond</a> to weight loss in general.”</p>
                                                <p>He continues to say that “no matter how you lose the weight, you'll have decreased visceral (belly) fat, decreased fasting blood sugars, decreased triglycerides, blood sugar, all those things.”  While most of these particular findings have only been confirmed in animal studies, Lowden seems to think that there’s good reason to assume people would experience similar benefits.</p>
                                            </li>
                                        </ul>
                                    </div>

                                    <button class="accordion">What’s the best way to manage hunger while fasting?</button>
                                    <div class="panel">
                                        <p>Remember, this isn’t a self-discipline contest :) If you get really hungry during your fast, you’re still allowed to eat.</p>
                                        <p>Most people find when gradually introducing intermittent fasting alongside the keto diet, they don’t get very hungry. In fact, it’s quite the opposite. Many people entirely lose their appetite.</p>
                                        <p>If you’re following this program closely, you’ll find nourishing broths are good at curbing your cravings and appetite, and coffee or tea with coconut oil or MCT oil really curbs the appetite and boosts energy levels.</p>
                                        <p>One of the simplest ways to reduce hunger is also one of the most effective: eat a teaspoon of coconut oil.</p>
                                        <p>When you don’t drink enough water your body can in fact give mixed signals on hunger. So dehydration can make you feel like you need to eat, when really you need to increase your liquid intake.</p>
                                    </div>

                                    <button class="accordion">What is the best way to break a fast and begin eating again?</button>
                                    <div class="panel">
                                        <p>One of the reasons we like supported fasting is because you’ll be much less likely to binge eat when you finally break your fast. Since your body won’t be deprived of nutrients, you won’t really feel like over-eating.</p>
                                        <p>We recommend choosing your favorite recipe from your Keto Meal Plan’s breakfast or lunch and breaking your fast with that meal. It can derail your weight loss efforts if you eat a massive feast of calories after a fast, especially if these are loaded with carbs.</p>
                                    </div>

                                    <button class="accordion">What are the best times in general to fast?</button>
                                    <div class="panel">
                                        <p>We find it’s better for your sleep cycle to eat an early dinner and skip breakfast rather than eat breakfast and skip dinner. So a 12-8pm eating window we find much better than say 8 am to 4 pm.<p>
                                        <p>However, feel free to experiment with different eating windows to see which works best for you. You also need to consider your school, work, or activity schedule, which could affect the time you're able to fast.<p>
                                        <p>So while there is nothing wrong with eating breakfast instead of dinner, we find it isn’t as good for our sleep quality as skipping breakfast. Feel free to see how this works for you, however.<p>
                                    </div>

                                    <button class="accordion">How and when do I use the drinks and soups recipes?</button>
                                    <div class="panel">
                                        <p>The cool thing about these keto-friendly soups and drinks is that you can pretty much consume them whenever you want. It’s pretty hard to break your ketosis when you’re eating soups that are virtually carb-free.<p>
                                        <p>Another plus is that soups tend to fill you up quickly. This means that you’ll probably fill yourself up long before you consume any significant number of calories.<p>
                                        <p>Of course, the caffeinated Keto Coffee and green tea should be consumed in the morning or early afternoon. Otherwise their stimulating effects might keep you awake.<p>
                                    </div>

                                    <button class="accordion">What electrolytes do you recommend?</button>
                                    <div class="panel">
                                        <p>All the electrolytes are important. The best thing is to find a good source of electrolytes (hint: you might want to avoid Gatorade).<p>
                                        <p>One such example is unprocessed coconut water. Coconuts grow in the tropics where people generally sweat all day, losing a ton of electrolytes in the process. They are nature’s way of helping humans replenish their lost electrolytes, and coconut water is available all over the place.<p>
                                        <p>Really, though, any sort of unsweetened electrolyte drink without any additives will help to replenish your electrolyte stores.<p>
                                    </div>

                                    <button class="accordion">I have almost no appetite and only want to eat one meal a day. Is that OK?</button>
                                    <div class="panel">
                                        <p>It’s normal for your appetite to go down when you’re on keto. Once your body stops craving and relying on carbs as a primary energy source, you realize that you don’t need to refuel as often. Longer-lasting energy sources like fats and proteins will keep you full and energized for longer.<p>
                                        <p>This means that plenty of people only need one meal a day when they’re on keto. However, if you feel like eating more, don’t worry about it! You’re not trying to count calories here.<p>
                                        <p>We recommend though, if you really, really feel like only eating one meal a day, aim to squeeze in a meal and snack over an hour of eating, and perhaps add an extra serving or two to the meal.<p>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="col-12">
                                    <div class="comment-inner">
                                        <div class="comment-title">
                                            Don't Find You Are Looking For? Ask Us A Question
                                        </div>

                                        <div class="comment-form-wrap">
                                            <div class="comment-respond" data-parent-id="0">
                                                <div class="comment-reply-title">
                                                    <a href="#" id="cancel-comment-reply-link">Cancel reply</a>
                                                </div>
                                                <form id="comment_form">
                                                    ...
                                                    <div class="form-group form-element">
                                                        ...
                                                        <input class="form-control" type="email" name="comment_email" value="" id='email' required/>
                                                    </div>

                                                    <div class="form-group comment-check-wrap">
                                                        <input type="checkbox" id="comment_check" name="comment_check" value="">
                                                        <label for="comment_check" class="noselect"> Save my name, email and website in this browser for the next time.</label><br>
                                                    </div>

                                                    <div class="form-group">
                                                        <textarea class="form-control" name="comment_body" value="" id='comment' rows="7" required></textarea>
                                                    </div>

                                                    <input type='hidden' name='parent_id' value="0" id='parent_id' />

                                                    <div class="form-group comment-btn-wrap submit_btn_group">
                                                        <button type="button" id="qa_submit_btn">Submit</button>
                                                        <div class="warning-text text-center"></div>
                                                    </div>
                                                </form>
                                            </div>


                                            <div class="comment-show-wrap">
                                                <div class="comment-show-count">
                                                    <?php
                                                    $total_comments = 0;
                                                    foreach ($comments as $comment_element) {
                                                        if ($comment_element->approved == 1) {
                                                            $total_comments += 1;
                                                        }
                                                    }
                                                    echo $total_comments;
                                                ...
                                                <div class="comment-show-list">
                                                    <?php
                                                    foreach ($comments as $comment_element) {
                                                        if ($comment_element->approved == 1 && $comment_element->parent_id == 0) {
                                                            $current_parent_id = $comment_element->id;
                                                            $reply_count = 0;
                                                            ?>   
                                                            <div class="comment-show-every" data-comment-id="<?php echo $comment_element->id ?>" data-comment-parent-id="<?php echo $comment_element->parent_id ?>">
                                                                <div class="comment-show-name"><?php echo $comment_element->name ?></div>
                                                                <div class="comment-show-time"><?php echo $comment_element->create_time ?></div>
                                                                <div class="comment-show-body"><?php echo $comment_element->comment ?></div>

                                                                <div class="comment-show-btns">
                                                                    <div class="comment-show-like-wrap">
                                                                        <a href="#" class="comment-like-btn" data-comment-id="<?php echo $comment_element->id ?>">Like</a>
                                                                        <span class="comment-like-count">
                                                                            <?php
                                                                            foreach ($comments_like as $comment_like) {
                                                                                if ($comment_element->id == $comment_like->comment_id) {
                                                                                    if ($comment_like->total_count > 0)
                                                                                        echo $comment_like->total_count;
                                                                                }
                                                                            }
                                                                            ?>
                                                                        </span>
                                                                    </div>
                                                                    <div class="comment-show-split"> | </div>
                                                                    <div class="comment-show-reply-wrap">
                                                                        <span class="comment-reply-count">
                                                                            <?php
                                                                            foreach ($comments as $count_element) {

                                                                                if ($count_element->parent_id == $current_parent_id) {
                                                                                    $reply_count++;
                                                                                }
                                                                            }
                                                                            if ($reply_count > 0)
                                                                        ...
                                                                        <a href="#" class="comment-reply-btn" data-comment-id="<?php echo $comment_element->id ?>">Reply</a>
                                                                        <span class="comment-reply-recent-time">
                                                                            <?php
                                                                            /*
                                                                              $time_minutes;
                                                                              foreach($comments as $count_element){

                                                                              $time_minutes = (strtotime($comment_element->comment_created) - strtotime($count_element->comment_created))/60;
                                                            ...
                                                            foreach ($comments as $comment_element) {
                                                                if ($comment_element->approved == 1) {
                                                                    if ($current_parent_id == $comment_element->parent_id) {
                                                                        $current_parent_parent_id = $comment_element->id;
                                                                        ?>
                                                                        <div class="comment-show-every" data-comment-id="<?php echo $comment_element->id ?>" data-comment-parent-id="<?php echo $comment_element->parent_id ?>">
                                                                            <div class="comment-show-name"><?php echo $comment_element->name ?></div>
                                                                            <div class="comment-show-time"><?php echo $comment_element->create_time ?></div>
                                                                            ...
                                                                            <div class="comment-show-btns">
                                                                                ...
                                                                                <div class="comment-show-reply-wrap">
                                                                                    <span class="comment-reply-count">
                                                                                        ...
                                                                                        <?php
                                                                                        $reply_second_count = 0;
                                                                                        foreach ($comments as $count_element) {

                                                                                            if ($count_element->parent_id == $comment_element->id) {
                                                                                                $reply_second_count++;
                                                                                            }
                                                                                        }
                                                                                        if ($reply_second_count > 0)
                                                                                            echo $reply_second_count;
                                                                                        ?>
                                                                                    </span>
                                                                                    <a href="#" class="comment-reply-btn" data-comment-id="<?php echo $comment_element->id ?>">Reply</a>
                                                                                    <span class="comment-reply-recent-time"></span>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                        <?php
                                                                        foreach ($comments as $comment_element) {
                                                                            if ($comment_element->approved == 1) {
                                                                                if ($current_parent_parent_id == $comment_element->parent_id) {
                                                                                    ?>
                                                                                    <div class="comment-show-every" data-comment-id="<?php echo $comment_element->id ?>" data-comment-parent-id="<?php echo $comment_element->parent_id ?>">
                                                                                        <div class="comment-show-name"><?php echo $comment_element->name ?></div>
                                                                                        <div class="comment-show-time"><?php echo $comment_element->create_time ?></div>
                                                                                        ...
                                                                                        <div class="comment-show-btns">
                                                                                            ...
                                                                                            <div class="comment-show-split"> | </div>
                                                                                            <div class="comment-show-reply-wrap">
                                                                                                <span class="comment-reply-count"></span>
                                                                                                <a href="#" class="comment-reply-btn" data-comment-id="<?php echo $comment_element->id ?>">Reply</a>
                                                                                                <span class="comment-reply-recent-time"></span>
                                                                                            </div>
                                                                                        </div>
                                        ...
                                        <?php
                                        /*
                                          echo json_encode($comments_like);

                                          foreach($comments_like as $comment_element){
                                          echo $comment_element->total_count;
                                          echo '<br>';
                                          }
                                         */
                                        ?>
...
if (!$obj_mealplan) {
    ...
    echo "var mealplan_object = " . json_encode($obj_mealplan) . ";";
}
?>
</script>
