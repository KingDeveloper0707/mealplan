...
class Admin_model extends CI_Model {
        ...
        public function updateGroups($postData=null, $action=null) {
                if ($action == "add") {
                    $error = 0;
                    if (!isset($postData["name"]) || empty($postData["name"])) { $error = 2;} else { $name = $this->db->escape(strip_tags($postData["name"])); $limitation = isset($postData["limitation"]) ? $postData["limitation"] : 0;}
                    ...
                    if ($query->num_rows() > 0) {
                        ...
                        $sql2 = "INSERT INTO admin_groups (`name`, `limitation`) VALUES (".$name.",".$limitation.")";
                        $this->db->query($sql2);
                        return TRUE;
                    }
                }
                if ($action == "edit") {
                    ...
                    if ($error == 2) { return $error; }
                    $sql = "SELECT * FROM admin_groups WHERE `name` = ".$name. " AND `limitation` = " . $limitation;
                    $query = $this->db->query($sql);
                    if ($query->num_rows() > 0) {
                        return 4;
                    } else {
                        $sql2 = "UPDATE admin_groups SET `name` = ".$name.", `limitation` = ". $limitation ." WHERE id = ".$id;
                        $this->db->query($sql2);
                        return TRUE;
                    }
                }
        ...
        public function adminLogin($postData) {
        	...
        	$sql = "SELECT admin.id, admin.username, admin.verification_key, admin_groups.limitation FROM admin LEFT JOIN admin_groups ON admin.admin_group = admin_groups.id WHERE admin.username = ".$username." AND admin.password = ".$password;
        	$query = $this->db->query($sql);
        	if ($query->num_rows() > 0) {
        		$q = $query->row();
